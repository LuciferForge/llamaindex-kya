"""KYAIdentityTool — A LlamaIndex FunctionTool for verifying agent identity cards.

Usable as a standalone tool or within a LlamaIndex agent's tool belt.
Works with or without llama-index installed.
"""
...
# Try importing LlamaIndex's FunctionTool. If not available, provide a fallback.
try:
...
def _verify_card_data(card: Dict[str, Any], public_key_path: Optional[str] = None) -> Dict[str, Any]:
    """Core verification logic, independent of LlamaIndex."""
...
def _make_kya_identity_tool():
    """Create a LlamaIndex FunctionTool wrapping verify_identity."""
    if _HAS_LLAMAINDEX:
...
# Build the tool class with a consistent interface for both installed/not-installed cases
if _HAS_LLAMAINDEX:

    class KYAIdentityTool:
        """KYA identity verification as a LlamaIndex FunctionTool.
        ...
        def as_tool():
            ...
            return _make_kya_identity_tool()

        @staticmethod
        def run(card_json: str, public_key_path: Optional[str] = None) -> str:
    ...
    class KYAIdentityTool:  # type: ignore[no-redef]
        """KYA identity verification tool (llama-index not installed — standalone mode)."""
