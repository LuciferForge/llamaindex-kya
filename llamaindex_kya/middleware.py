"""Middleware — @kya_verified decorator for gating LlamaIndex agent execution.

Wraps a LlamaIndex query/chat callback or any callable to require KYA identity
...
from llamaindex_kya.card import get_card
from llamaindex_kya.identity import _verify_card_data
...
def kya_verified(
    ...
    Usage:
        ...
        def sensitive_query(agent, query):
            ...

        @kya_verified(min_score=60)
        def my_tool_callback(output):
            ...
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if agent is None and args:
                # Check if first arg looks like a LlamaIndex agent
                # (has tools attribute or _kya_card)
                candidate = args[0]
                if hasattr(candidate, "tools") or hasattr(candidate, "_kya_card"):
                    agent = candidate
...
def _handle_fail(
    ...
    if on_fail == "raise":
        ...
        print(
            f"[llamaindex-kya] WARNING: {agent_name} — {reason}",
            file=sys.stderr,
        )
        return func(*args, **kwargs)
    else:
